...
Công cụ cào dữ liệu doanh nghiệp trên **Google Maps** theo danh sách từ khoá tìm kiếm tuỳ chọn, tự động lọc các doanh nghiệp **đang hoạt động**, lưu trữ an toàn vào **SQLite**, và đóng gói hoàn chỉnh bằng **Docker**.
...
> - **Tìm kiếm linh hoạt theo từ khóa**: Không gán cứng khu vực, kết quả phụ thuộc hoàn toàn vào từ khóa bạn cấu hình (tỉnh thành, quận huyện, ngành nghề,...).
...
_File cơ sở dữ liệu `businesses.db` sẽ tự động được lưu và cập nhật trong thư mục `./data/` trên máy tính của bạn._
...
_(File CSV/JSON sẽ xuất hiện ngay trong thư mục `./data/` trên máy host)_
